...
@section('title', 'Tambah Presensi Akademik')
    ...
    <section class="bg-white py-4 rounded-lg mb-4 antialiased md:py-8">
        <div class="mx-auto max-w-screen-xl px-4 2xl:px-0">
            <div class="mx-auto max-w-5xl">
                <form action="{{ route('presensi-akademik.store') }}" method="POST">
                    @csrf
                    <div class="mb-4">
                        <label for="hari" class="block text-sm font-medium text-gray-700">Hari</label>
                        <input type="text" name="hari" id="hari"
                            class="block w-full mt-1 border-gray-300 rounded-md shadow-sm" value="{{ old('hari') }}"
                            required readonly>
                        @error('hari')
                    ...
                    <div class="mb-4">
                        ...
                        <input type="date" name="tanggal" id="tanggal"
                            class="block w-full mt-1 border-gray-300 rounded-md shadow-sm" value="{{ old('tanggal') }}"
                            required>
                        @error('tanggal')
                            <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
                        @enderror
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        tanggalInput.addEventListener('change', function() {
            ...
            console.log('Tanggal dipilih:', tanggal); // Untuk debug
            const hari = getDayFromDate(tanggal);
            console.log('Hari:', hari); // Untuk debug
