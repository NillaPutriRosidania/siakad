    ...
    <div class="bg-white p-2 mb-4 border-2 border-white rounded-lg">
        <div class="flex justify-center items-center rounded-lg border-gray-300 h-4">
            <h2 class="flex items-center justify-center text-sm text-center lg:text-lg font-bold uppercase text-red-600">
                Daftar Presensi Akademik
            </h2>
        </div>
    </div>

    <div class="mt-4">
        <form method="GET" action="{{ route('presensi-akademik.index') }}">
            <div class="flex justify-between items-center mx-28">
                <div class="flex gap-2">
                    <a href="{{ route('presensi-akademik.create') }}"
                ...
                <div class="flex items-center">
                    <input type="text" name="search" class="w-full p-2 border border-gray-300 rounded-lg mx-4"
                        placeholder="Cari Presensi..." value="{{ request('search') }}">
            ...
            <div class="mx-auto max-w-5xl">
                    ...
                    <table class="min-w-full divide-y divide-gray-200">
                        <thead class="bg-red-600">
                            <tr>
                                ...
                                <th class="px-6 py-3 text-xs font-medium text-white uppercase tracking-wider">Mata Kuliah
                                </th>
                                <th class="px-6 py-3 text-xs font-medium text-white uppercase tracking-wider">Tanggal</th>
                                <th class="px-6 py-3 text-xs font-medium text-white uppercase tracking-wider">Status
                                    Kehadiran</th>
                        ...
                        <tbody class="bg-white">
                            @foreach ($presensiList as $presensiItem)
                                <tr class="border-b border-gray-200">
                                    <td class="px-6 py-4">{{ $loop->iteration }}</td>
                                    <td class="px-6 py-4">{{ $presensiItem->mahasiswa->NIM }}</td>
                                    <td class="px-6 py-4">{{ $presensiItem->mahasiswa->Nama }}</td>
                                    <td class="px-6 py-4">{{ $presensiItem->matakuliah->Nama_mk }}</td>
                                    <td class="px-6 py-4">{{ $presensiItem->tanggal }}</td>
                                    <td class="px-6 py-4">{{ ucfirst($presensiItem->status_kehadiran) }}</td>
                                    <td class="px-6 py-4 text-center">
                                        <div class="flex justify-center items-center gap-2">
                                            <form action="{{ route('presensi-akademik.destroy', $presensiItem->id) }}"
                                            ...
                                            <a href="{{ route('presensi-akademik.edit', $presensiItem->id) }}"
                ...
                @else
                    <div class="text-center">
                        <h1 class="text-red-600 font-bold">Data presensi akademik masih kosong</h1>
                    </div>
                @endif
            </div>
        </div>
