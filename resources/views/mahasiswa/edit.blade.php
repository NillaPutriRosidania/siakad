            ...
            <div class="mx-auto max-w-5xl">
                <form action="{{ route('mahasiswa.update', $mahasiswa->NIM) }}" method="POST">
                    ...
                    <div class="mb-4">
                        ...
                        <textarea id="Alamat" name="Alamat" class="block w-full mt-1 border-gray-300 rounded-md shadow-sm">{{ old('Alamat', $mahasiswa->Alamat) }}</textarea>
                        @error('Alamat')
                            <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="mb-4">
                        ...
                        <input type="text" id="Nohp" name="Nohp" value="{{ old('Nohp', $mahasiswa->Nohp) }}"
                            class="block w-full mt-1 border-gray-300 rounded-md shadow-sm">
                        @error('Nohp')
                            <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
                        @enderror
                        ...
                        <select id="id_Gol" name="id_Gol"
                            ...
                            @foreach ($golonganList as $gol)
                                <option value="{{ $gol->id_Gol }}"
                                    {{ old('id_Gol', $mahasiswa->id_Gol) == $gol->id_Gol ? 'selected' : '' }}>
                                    {{ $gol->nama_Gol }}
                                </option>
                            @endforeach
                        </select>
